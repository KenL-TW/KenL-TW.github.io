(() => {
  const DEFAULT_CONFIG = {
    ...
    GITHUB_BRANCH: "main",
    GITHUB_PAGES_BASE: "", // e.g. "https://ken-personal-page.digitalaaronl.workers.dev" (optional)
  ...
  const css = `
  :root{ --dtz:2147483000; }
  .dt-chatbot, .dt-chatbot * { box-sizing: border-box; font-family: Inter, ui-sans-serif, system-ui, -apple-system, Segoe UI, sans-serif; }
  .dt-chatbot { position: fixed; right: 18px; top: auto; bottom: 18px; transform: none; z-index: var(--dtz); }

  .dt-chatbot[data-theme="light"]{
    --bg: #111111; --bg2:#111111;
    --card:#ffffff; --soft:#f6f6f3;
    --line: #d9d9d9;
    --text: #111111; --muted: #5e5e5e;
    --shadow: 0 18px 54px rgba(17,17,17,.16);
    --shadow2: 0 10px 28px rgba(17,17,17,.18);
    --chip: #ffffff;
    --chipHover: #f1f1ed;
    --btn: #1f4eff;
    --btnText: #ffffff;
  ...
  .dt-fab svg{ width: 22px; height: 22px; fill:none; stroke:currentColor; stroke-width:1.8; }
  .dt-fab:hover{ filter: brightness(1.08); }
  .dt-fab:active{ transform: scale(.98); }
  .dt-fab:focus-visible,
  .dt-iconbtn:focus-visible,
  .dt-actBtn:focus-visible,
  .dt-chip:focus-visible,
  .dt-send:focus-visible,
  .dt-jump:focus-visible{
    outline: 3px solid #ff5a2f;
    outline-offset: 3px;
  }
  ...
  .dt-iconbtn svg{ width:16px; height:16px; fill:none; stroke:currentColor; stroke-width:1.8; }
  ...
  .dt-skeleton{
    background: var(--chipHover);
    animation: dt-shimmer 1.1s infinite alternate;
    border-radius: 0;
    height: 12px;
    margin: 4px 0;
  }
  ...
  .dt-send{
    ...
    border-radius: 0;
    border: 1px solid var(--line);
    background: ${THEME === "dark" ? "#fff" : "#1f4eff"};
  ...
  root.innerHTML = `
    <button class="dt-fab" type="button" aria-label="Open Ask Ken">
    ...
    <div class="dt-panel" role="dialog" aria-label="Chatbot Panel">
      <div class="dt-header">
        ...
        <div class="dt-actions">
          ${modeToggleHtml}
          <button class="dt-iconbtn" type="button" data-action="clear" title="Clear conversation" aria-label="Clear conversation">
            <svg viewBox="0 0 20 20" aria-hidden="true">
              <path d="M4 6h12M8 3h4l1 3H7l1-3ZM6 6l1 11h6l1-11"></path>
            </svg>
          </button>
          <button class="dt-iconbtn" type="button" data-action="close" title="Close" aria-label="Close Ask Ken">
            <svg viewBox="0 0 20 20" aria-hidden="true">
      ...
      <div class="dt-body" data-slot="body">
        <button class="dt-jump" type="button" data-action="jump">
          <svg viewBox="0 0 16 16" width="14" height="14" aria-hidden="true">
            <path d="M8 2v10M4 8l4 4 4-4" fill="none" stroke="currentColor" stroke-width="1.5"></path>
          </svg>
          回到最新
        </button>
      </div>
  ...
  function renderTopActions() {
    ...
    list.forEach((item, idx) => {
      ...
      btn.innerHTML = `
        <div class="dt-actIcon">${escapeHtml(item.icon ?? String(idx + 1).padStart(2, "0"))}</div>
  ...
  root.addEventListener("click", (e) => {
    ...
    const act = actionElement.getAttribute("data-action");
    if (!act) return;
  ...
  function addMessage(role, text, citations, cards) {
    ...
    if (role !== "user" && CFG.SHOW_FEEDBACK) {
      ...
      helpfulBtn.innerHTML = '<svg viewBox="0 0 20 20" width="16" height="16" aria-hidden="true"><path d="M7 9 10 3c1 0 2 1 2 2v3h4c1 0 2 1 1.7 2l-1.4 6H7V9ZM3 9h4v8H3V9Z" fill="none" stroke="currentColor" stroke-width="1.5"/></svg>';
      ...
      notHelpfulBtn.innerHTML = '<svg viewBox="0 0 20 20" width="16" height="16" aria-hidden="true"><path d="m7 11 3 6c1 0 2-1 2-2v-3h4c1 0 2-1 1.7-2l-1.4-6H7v7ZM3 3h4v8H3V3Z" fill="none" stroke="currentColor" stroke-width="1.5"/></svg>';
